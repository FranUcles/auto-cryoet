...
def process_class(_input, xsize, metadata_df, reference_temb_df, outputDir):
    ...
    outputfilename = Path(_input).stem
    outputfile = Path(outputDir) / (outputfilename + ".temb" )
    logger.debug(f"Saving file {str(outputfile.resolve())}")
    save(subset_temb, str(outputfile.resolve()))
    logger.info("Converted!")

def main(input, imgXsize, inputDir, referenceEmb, metadata, outputDir=".", debug=False, quiet=False, no_logs=False):
    ...
    if inputDir is None:
        process_class(input, imgXsize, metadata, reference_temb, outputDir)
    else:
        for file in os.listdir(inputDir):
            file_path = Path(inputDir) / file
            process_class(str(file_path.resolve()), imgXsize, metadata, reference_temb, outputDir)
